...
export const App = () => {
  ...
  const validateSubmission = (e: FormEvent<HTMLFormElement>) => {
    ...
    const values: string[] = []
    formData.forEach((value) => { values.push(value as string)})
  ...
  return (
    <div className='flex w-full justify-center items-center h-screen bg-slate-100'>
      <div className='md:w-2/3 lg:w-1/3 border-gray-300 bg-white border p-8 rounded'>
        ...
        <form action={`${import.meta.env.VITE_API_URL}/signin`} method="post" onSubmit={validateSubmission} id="sign-in" className='hidden'>
        ...
        {activeAccount ? (
          <div className='mt-6'>
            ...
            <div className='bg-green-100 p-4 mb-1 rounded text-green-900 text-sm'>
              ...
              <div className='text-xs italic mt-1'><span className='font-semibold'>Public Key:</span> <span className='break-all'>{activeAccount.publicKey}</span></div>
              <div className='text-xs italic mt-1'><span className='font-semibold'>Signature:</span> <span className='break-all'>{signature ? signature : 'Message not yet signed'}</span></div>
            </div>
          </div>
        ) : (
            ...
            <div className='text-sm'>
              {map((account: AccountInfo) => (
                <div key={account.address} className='bg-slate-100 p-4 mb-1 rounded'>
                  ...
                  <div className='text-xs italic'><span className='break-all'>{account.publicKey}</span></div>
                </div>
              ))(accounts)}
            </div>
          </div>
        ...
        {activeAccount ? (
          <div className='flex flex-col mt-4'>
            {signature ? <button type='submit' form='sign-in' className='bg-green-500 hover:bg-green-400 text-white font-bold py-2 px-4 border-b-4 border-green-700 hover:border-green-500 rounded w-full mb-2'>Complete sign in</button> : <button onClick={() => handleSignin()} className='bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded w-full mb-2'>Sign Message</button>}
            <button onClick={() => disconnect()} className='hover:underline underline-offset-2 text-gray-800 py-2 px-4 rounded w-full text-center'>Disconnect</button>
          </div>
        ) : (
          <div>
            <button onClick={() => handleConnect()} className='bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded w-full mb-2'>Connect</button>
          </div>
        )}
      </div>
    </div>
